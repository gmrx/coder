...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    details: [
      'Ищет SKILL.md в пользовательских skill roots. Workspace skill roots читаются только если уже существуют, расширение их не создаёт.',
  ...
  },
  {
    ...
    details: [
      'Читает MCP конфиг из внутренних настроек расширения aiAssistant.mcpServers.',
  ...
  },
  {
    ...
    summary: 'Читает список проектов из Jira через REST API настроенного сервиса задач.',
    details: [
      'Использует настройки Jira из панели расширения: host, login и password.',
      'Без логина и пароля выполняет anonymous-запрос и увидит только публично доступные проекты.',
      'Подходит как первый шаг перед поиском задач по конкретному project key.',
    ],
    args: [
      { name: 'query', description: 'фильтр по key, name, type или lead' },
      { name: 'limit', description: 'сколько проектов вернуть, по умолчанию 100' },
      { name: 'offset', description: 'смещение для локальной пагинации' },
      { name: 'baseUrl', description: 'необязательный override Jira host; обычно используй настройки' },
    ],
    examples: [
      '{ "tool": "jira_list_projects", "args": { "limit": 50 } }',
      '{ "tool": "jira_list_projects", "args": { "query": "OPS" } }',
    ],
    aliases: ['jira_projects', 'list_jira_projects', 'read_jira_projects', 'jira_project_list'],
    searchHints: ['Jira проекты', 'получить мои проекты', 'список проектов сервиса задач', 'jira project key', 'проекты из настроенной Jira'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Проекты Jira',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['нужно получить проекты из Jira или подобрать project key перед поиском задач'],
      guidance: 'Сначала вызови без фильтра или с query, затем используй key проекта в jira_search_tasks.',
    },
  },
  {
    name: 'jira_search_tasks',
    summary: 'Ищет и читает задачи Jira по JQL или простым фильтрам.',
    details: [
      'Использует Jira host/login/password из настроек расширения.',
      'Если jql передан явно, выполняет его как есть. Если jql нет, строит запрос из project, assignee, status и text.',
      'По умолчанию сортирует задачи по updated DESC и возвращает краткие поля.',
    ],
    args: [
      { name: 'jql', description: 'полный JQL-запрос, например project = "ABC" ORDER BY updated DESC' },
      { name: 'project', description: 'project key или имя проекта для JQL project = ...' },
      { name: 'assignee', description: 'исполнитель; можно me или currentUser()' },
      { name: 'status', description: 'статус или массив статусов' },
      { name: 'text', description: 'поиск по тексту через JQL text ~ ...' },
      { name: 'limit', description: 'сколько задач вернуть, по умолчанию 10, максимум 50' },
      { name: 'offset', description: 'startAt для пагинации' },
      { name: 'fields', description: 'список Jira fields строкой через запятую или массивом' },
      { name: 'baseUrl', description: 'необязательный override Jira host; обычно используй настройки' },
    ],
    examples: [
      '{ "tool": "jira_search_tasks", "args": { "jql": "assignee = currentUser() ORDER BY updated DESC", "limit": 10 } }',
      '{ "tool": "jira_search_tasks", "args": { "project": "ABC", "status": "Open", "limit": 20 } }',
    ],
    aliases: ['jira_tasks', 'read_jira_tasks', 'search_jira_issues', 'jira_search_issues', 'jira_issues'],
    searchHints: ['Jira задачи', 'получить мои задачи', 'прочитать задачи из сервиса', 'найти jira issue', 'JQL', 'задачи из настроенной Jira'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Задачи Jira',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['нужно получить список задач из Jira, найти задачи пользователя или проекта'],
      guidance: 'Если пользователь просит "мои задачи", используй assignee=me. Если project key неизвестен, сначала вызови jira_list_projects.',
    },
  },
  {
    name: 'jira_get_task',
    summary: 'Читает подробности одной задачи Jira по key, например ABC-123.',
    details: [
      'Возвращает summary, статус, участников, описание, связи, эпик, вложения, дополнительные поля и комментарии, если поля доступны.',
      'Использует Jira host/login/password из настроек расширения.',
    ],
    args: [
      { name: 'key', description: 'ключ задачи Jira, например ABC-123', required: true },
      { name: 'fields', description: 'список Jira fields строкой через запятую или массивом' },
      { name: 'baseUrl', description: 'необязательный override Jira host; обычно используй настройки' },
    ],
    examples: [
      '{ "tool": "jira_get_task", "args": { "key": "ABC-123" } }',
    ],
    aliases: ['jira_issue', 'get_jira_issue', 'read_jira_issue', 'jira_task', 'get_jira_task'],
    searchHints: ['Jira issue по ключу', 'прочитать задачу Jira', 'детали задачи', 'ABC-123', 'карточка задачи Jira'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Задача Jira',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['известен key задачи и нужны подробности'],
      avoidWhen: ['нужно сначала найти задачу среди нескольких — тогда начни с jira_search_tasks'],
      guidance: 'Используй после jira_search_tasks, когда нужно раскрыть конкретную задачу.',
    },
  },
  {
    name: 'tfs_list_projects',
    summary: 'Читает список проектов из TFS 2015 через REST API настроенной коллекции.',
    details: [
      'Использует настройки TFS из панели расширения: host, collection, login и password.',
      'Для короткого логина умеет повторить Basic-запрос как DOMAIN\\login, если домен пришёл в NTLM challenge.',
      'Подходит как первый шаг перед поиском work items по конкретному Team Project.',
    ],
    args: [
      { name: 'query', description: 'фильтр по имени, описанию, id или состоянию проекта' },
      { name: 'limit', description: 'сколько проектов вернуть, по умолчанию 100' },
      { name: 'offset', description: 'смещение для локальной пагинации' },
      { name: 'baseUrl', description: 'необязательный override TFS host; обычно используй настройки' },
      { name: 'collection', description: 'необязательный override коллекции TFS, например DefaultCollection' },
    ],
    examples: [
      '{ "tool": "tfs_list_projects", "args": { "limit": 50 } }',
      '{ "tool": "tfs_list_projects", "args": { "query": "IID" } }',
    ],
    aliases: ['tfs_projects', 'list_tfs_projects', 'read_tfs_projects', 'tfs_project_list'],
    searchHints: ['TFS проекты', 'Team Foundation Server проекты', 'получить проекты TFS', 'список проектов TFS', 'DefaultCollection'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Проекты TFS',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['нужно получить проекты из TFS или подобрать Team Project перед поиском work items'],
      guidance: 'Сначала вызови без фильтра или с query, затем используй project в tfs_search_tasks.',
    },
  },
  {
    name: 'tfs_search_tasks',
    summary: 'Ищет и читает work items TFS через WIQL и REST API.',
    details: [
      'Использует TFS host/collection/login/password из настроек расширения.',
      'Если wiql передан явно, выполняет его как есть. Если wiql нет, строит WIQL из project, assignee, status/state и text.',
      'По умолчанию ищет work items, назначенные на текущего пользователя через [System.AssignedTo] = @Me.',
    ],
    args: [
      { name: 'wiql', description: 'полный WIQL-запрос, например SELECT [System.Id] FROM WorkItems WHERE [System.AssignedTo] = @Me' },
      { name: 'project', description: 'Team Project для условия [System.TeamProject] = ...' },
      { name: 'assignee', description: 'исполнитель; можно me или @Me, по умолчанию me' },
      { name: 'status', description: 'состояние work item или массив состояний; синоним state' },
      { name: 'text', description: 'поиск по System.Title/System.Description через WIQL CONTAINS' },
      { name: 'limit', description: 'сколько work items вернуть, по умолчанию 10, максимум 50' },
      { name: 'offset', description: 'смещение для локальной пагинации' },
      { name: 'baseUrl', description: 'необязательный override TFS host; обычно используй настройки' },
      { name: 'collection', description: 'необязательный override коллекции TFS' },
    ],
    examples: [
      '{ "tool": "tfs_search_tasks", "args": { "assignee": "me", "limit": 10 } }',
      '{ "tool": "tfs_search_tasks", "args": { "project": "IID-744", "status": "Активно", "limit": 20 } }',
    ],
    aliases: ['tfs_tasks', 'tfs_work_items', 'read_tfs_tasks', 'search_tfs_work_items', 'tfs_wiql'],
    searchHints: ['TFS задачи', 'TFS work items', 'WIQL', 'получить мои задачи TFS', 'Team Foundation Server задачи'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Задачи TFS',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['нужно получить список work items из TFS, найти задачи пользователя или проекта'],
      guidance: 'Если пользователь просит "мои задачи TFS", используй assignee=me. Если project неизвестен, сначала вызови tfs_list_projects.',
    },
  },
  {
    name: 'tfs_get_task',
    summary: 'Читает подробности одного work item TFS по числовому id.',
    details: [
      'Возвращает title, состояние, участников, описание, связи, историю/комментарии из updates и дополнительные поля.',
      'Использует TFS host/collection/login/password из настроек расширения.',
    ],
    args: [
      { name: 'id', description: 'числовой id work item TFS, например 37678', required: true },
      { name: 'baseUrl', description: 'необязательный override TFS host; обычно используй настройки' },
      { name: 'collection', description: 'необязательный override коллекции TFS' },
    ],
    examples: [
      '{ "tool": "tfs_get_task", "args": { "id": 37678 } }',
    ],
    aliases: ['tfs_task', 'tfs_work_item', 'get_tfs_task', 'read_tfs_work_item'],
    searchHints: ['TFS work item по id', 'прочитать задачу TFS', 'детали work item', 'TFS карточка задачи'],
    subagentModes: ALL_SUBAGENT_MODES,
    workflowRoles: ['recommendation_redirect_source', 'retrieval'],
    capabilities: {
      userFacingName: 'Задача TFS',
      readOnly: true,
      concurrencySafe: true,
      shouldDefer: true,
    },
    prompt: {
      group: 'specialized',
      style: 'detailed',
      whenToUse: ['известен id work item и нужны подробности'],
      avoidWhen: ['нужно сначала найти задачу среди нескольких — тогда начни с tfs_search_tasks'],
      guidance: 'Используй после tfs_search_tasks, когда нужно раскрыть конкретный work item.',
    },
  },
  {
    name: 'shell',
