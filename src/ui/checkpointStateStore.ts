...
import type { CheckpointControllerState } from './checkpoints';

export class CheckpointStateStore {
  constructor(private readonly context: vscode.ExtensionContext) {}

  read(conversationId: string): CheckpointControllerState | null {
    const filePath = this.getFilePath(conversationId);
    if (!filePath) return null;
    try {
      const raw = fs.readFileSync(filePath, 'utf8');
      const parsed = JSON.parse(raw);
      return parsed && typeof parsed === 'object' ? parsed as CheckpointControllerState : null;
    } catch {
      return null;
    }
  }

  async write(conversationId: string, state: CheckpointControllerState): Promise<void> {
    const filePath = this.getFilePath(conversationId);
    if (!filePath) return;
  ...
  private getFilePath(conversationId: string): string {
    ...
    return path.join(storageUri.fsPath, 'checkpoint-states', `${safeId}.json`);
  }
}
